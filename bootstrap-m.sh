...
if [[ "$EUID" -ne 0 ]]; then
	echo "Reap bootstrap script must be run as root."
	echo
	exit 1
elif [[ $# != 1 || ! -d "$1" ]]; then
...
mkdir -p $DRAG_ROOT/root/.cache/reap-bootstrap
mkdir -p ~/.cache
ln -sf $DRAG_ROOT/root/.cache/reap-bootstrap ~/.cache
export PATH=$DRAG_ROOT/tools/bin:$PATH
export TGT=x86_64-reap-linux-gnu
export TGT32=i686-reap-linux-gnu
export TGTX32=x86_64-reap-linux-gnux32
export LC_ALL=POSIX
export CONFIG_SITE=$DRAG_ROOT/usr/share/config.site
unset CFLAGS CXXFLAGS
ashtray=~/.cache/drag/ashtray
if [ -f ~/.cache/reap-bootstrap/krnlver ]; then
	KRNLVER=$(cat ~/.cache/reap-bootstrap/krnlver)
	GLIBCVER=$(cat ~/.cache/reap-bootstrap/glibcver)
	GCCVER=$(cat ~/.cache/reap-bootstrap/gccver)
fi
...
if [ ! -f ~/.cache/reap-bootstrap/1 ]; then
echo
echo "STAGE 1 - Setup"
echo
...
NAME="Reap Linux"
PRETTY_NAME="Reap Linux"
ID="reap"
EOF

touch ~/.cache/reap-bootstrap/1
fi
if [ ! -f ~/.cache/reap-bootstrap/2 ]; then
echo
...
#curl -Lo ~/.cache/reap-bootstrap/lfs.html https://linuxfromscratch.org/lfs/view/stable/chapter03/packages.html
#DGCCVER=$(grep ">GCC (" ~/.cache/reap-bootstrap/lfs.html)
#DGCCVER=${DGCCVER##*\(}
#DGCCVER=${DGCCVER%%)*}
#DGLIBCVER=$(grep ">Glibc (" ~/.cache/reap-bootstrap/lfs.html)
#DGLIBCVER=${DGLIBCVER##*\(}
#DGLIBCVER=${DGLIBCVER%%)*}
#DKRNLVER=$(grep ">Linux (" ~/.cache/reap-bootstrap/lfs.html)
#DKRNLVER=${DKRNLVER##*\(}
#DKRNLVER=${DKRNLVER%%)*}
#rm ~/.cache/reap-bootstrap/lfs.html
...
echo $KRNLVER  > ~/.cache/reap-bootstrap/krnlver
echo $GLIBCVER > ~/.cache/reap-bootstrap/glibcver
echo $GCCVER   > ~/.cache/reap-bootstrap/gccver

touch ~/.cache/reap-bootstrap/2
fi
if [ ! -f ~/.cache/reap-bootstrap/3 ]; then
echo
...
echo 'SUBSYSTEM=="video4linux", GROUP="video", MODE="0660"' > $pkgdir/etc/udev/rules.d/99-video.rules
echo 'SUBSYSTEM=="sound", GROUP="audio", MODE="0660"' > $pkgdir/etc/udev/rules.d/90-alsa.rules
}
EOF
...
sed -i '/b2sums=(.*)/d; /b2sums=(/,/)/d; /sha256sums=(.*)/d; /sha256sums=(/,/)/d; /sha512sums=(.*)/d; /sha512sums=(/,/)/d;' ~/.cache/drag/stash/{coreutils,diffutils,file,findutils,grep,gzip,patch,flex,attr,acl,psmisc,libtool,lib32-libltdl,inetutils,automake,groff,shadow,tcl,gmp,lib32-gmp,make}/PKGBUILD
...
for i in {coreutils,diffutils,findutils,grep,gzip,patch,flex,attr,acl,psmisc,libtool,lib32-libltdl,inetutils,automake,groff,shadow}; do
...
touch ~/.cache/reap-bootstrap/3
fi
if [ ! -f ~/.cache/reap-bootstrap/4 ]; then
echo
...
for i in ${cigs[@]}; do
	[[ $(cat ~/.cache/reap-bootstrap/3) != *"--$i--"* ]] && pinch $i
	echo "--$i--" >> ~/.cache/reap-bootstrap/3
done
...
touch ~/.cache/reap-bootstrap/4
fi
if [ ! -f ~/.cache/reap-bootstrap/5 ]; then
echo
...
touch ~/.cache/reap-bootstrap/5
fi
if [ ! -f ~/.cache/reap-bootstrap/6 ]; then
echo
...
touch ~/.cache/reap-bootstrap/6
fi
if [ ! -f ~/.cache/reap-bootstrap/7 ]; then
echo
...
sed -i 's/EXTRAVERSION =/EXTRAVERSION = -reap/' linux-$KRNLVER/Makefile

touch ~/.cache/reap-bootstrap/7
fi
if [ ! -f ~/.cache/reap-bootstrap/8 ]; then
echo
echo "STAGE 8 - Glibc"
echo
...
touch ~/.cache/reap-bootstrap/8
fi
if [ ! -f ~/.cache/reap-bootstrap/9 ]; then
echo
...
touch ~/.cache/reap-bootstrap/9
fi
if [ ! -f ~/.cache/reap-bootstrap/10 ]; then
echo
echo "STAGE 10 - m4"
echo
...
touch ~/.cache/reap-bootstrap/10
fi
if [ ! -f ~/.cache/reap-bootstrap/11 ]; then
echo
...
touch ~/.cache/reap-bootstrap/11
fi
if [ ! -f ~/.cache/reap-bootstrap/12 ]; then
echo
echo "STAGE 12 - bash"
echo
...
touch ~/.cache/reap-bootstrap/12
fi
if [ ! -f ~/.cache/reap-bootstrap/13 ]; then
echo
...
touch ~/.cache/reap-bootstrap/13
fi
if [ ! -f ~/.cache/reap-bootstrap/14 ]; then
echo
...
touch ~/.cache/reap-bootstrap/14
fi
if [ ! -f ~/.cache/reap-bootstrap/15 ]; then
echo
echo "STAGE 15 - file"
echo
...
touch ~/.cache/reap-bootstrap/15
fi
if [ ! -f ~/.cache/reap-bootstrap/16 ]; then
echo
...
touch ~/.cache/reap-bootstrap/16
fi
if [ ! -f ~/.cache/reap-bootstrap/17 ]; then
echo
echo "STAGE 17 - gawk"
echo
...
touch ~/.cache/reap-bootstrap/17
fi
if [ ! -f ~/.cache/reap-bootstrap/18 ]; then
echo
echo "STAGE 18 - grep"
echo
...
touch ~/.cache/reap-bootstrap/18
fi
if [ ! -f ~/.cache/reap-bootstrap/19 ]; then
echo
echo "STAGE 19 - gzip"
echo
...
touch ~/.cache/reap-bootstrap/19
fi
if [ ! -f ~/.cache/reap-bootstrap/20 ]; then
echo
echo "STAGE 20 - make"
echo
...
touch ~/.cache/reap-bootstrap/20
fi
if [ ! -f ~/.cache/reap-bootstrap/21 ]; then
echo
echo "STAGE 21 - patch"
echo
...
touch ~/.cache/reap-bootstrap/21
fi
if [ ! -f ~/.cache/reap-bootstrap/22 ]; then
echo
echo "STAGE 22 - sed"
echo
...
touch ~/.cache/reap-bootstrap/22
fi
if [ ! -f ~/.cache/reap-bootstrap/23 ]; then
echo
echo "STAGE 23 - tar"
echo
...
touch ~/.cache/reap-bootstrap/23
fi
if [ ! -f ~/.cache/reap-bootstrap/24 ]; then
echo
echo "STAGE 24 - xz"
echo
...
touch ~/.cache/reap-bootstrap/24
fi
if [ ! -f ~/.cache/reap-bootstrap/25 ]; then
echo
...
touch ~/.cache/reap-bootstrap/25
fi
if [ ! -f ~/.cache/reap-bootstrap/26 ]; then
echo
...
touch ~/.cache/reap-bootstrap/26
fi
if [ ! -f ~/.cache/reap-bootstrap/27 ]; then
echo
...
touch ~/.cache/reap-bootstrap/27
fi
if [ ! -f ~/.cache/reap-bootstrap/28 ]; then
echo
echo "STAGE 28 - bison"
echo
...
touch ~/.cache/reap-bootstrap/28
fi
if [ ! -f ~/.cache/reap-bootstrap/29 ]; then
echo
echo "STAGE 29 - perl"
echo
...
touch ~/.cache/reap-bootstrap/29
fi
if [ ! -f ~/.cache/reap-bootstrap/30 ]; then
echo
...
touch ~/.cache/reap-bootstrap/30
fi
if [ ! -f ~/.cache/reap-bootstrap/31 ]; then
echo
...
touch ~/.cache/reap-bootstrap/31
fi
if [ ! -f ~/.cache/reap-bootstrap/32 ]; then
echo
...
touch ~/.cache/reap-bootstrap/32
fi
if [ ! -f ~/.cache/reap-bootstrap/33 ]; then
echo
...
find $DRAG_ROOT/usr -depth -name x86_64-reap-linux-gnu\* | xargs rm -rf
sed -i 's/#//' $DRAG_ROOT/etc/profile

touch ~/.cache/reap-bootstrap/33
fi
